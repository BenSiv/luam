...
if [ -d "lib/lfs/src" ]; then
    ...
    run_cmd gcc -O2 -shared -fPIC -Isrc/ -o lib/lfs/lfs.so lib/lfs/src/lfs.c
    if [ $? -eq 0 ]; then
...
if [ -d "lib/yaml" ]; then
    ...
    run_cmd gcc -O2 -shared -fPIC -Isrc/ -Ilib/yaml -o lib/yaml/yaml.so lib/yaml/*.c
    if [ $? -eq 0 ]; then
...
if [ -d "lib/sqlite" ]; then
    ...
    run_cmd gcc -O2 -shared -fPIC -Isrc/ -o lib/sqlite/lsqlite3.so lib/sqlite/lsqlite3.c -lsqlite3
    if [ $? -eq 0 ]; then
...
if [ -d "lib/socket/src" ]; then
    ...
    run_cmd cp lib/socket/src/socket.lua lib/socket/init.lua
    run_cmd cp lib/socket/src/mime.lua lib/mime/init.lua
    run_cmd cp lib/socket/src/ltn12.lua lib/ltn12/init.lua
    run_cmd cp lib/socket/src/{ftp,http,smtp,tp,url,headers,mbox}.lua lib/socket/

    # Compile Socket Core (Linux files)
    run_cmd gcc -O2 -shared -fPIC -Isrc/ -Ilib/socket/src \
        -DLUASOCKET_DEBUG \
...
if [ -d "lib/ssl/src" ]; then
    ...
    if [ ! -d "$OPENSSL_DIR" ]; then
        ...
        if [ ! -f "dep/$OPENSSL_TAR" ]; then
            cd dep
            run_cmd wget "https://www.openssl.org/source/$OPENSSL_TAR" || run_cmd curl -O "https://www.openssl.org/source/$OPENSSL_TAR"
            cd ..
        fi
        echo "Extracting OpenSSL"
        cd dep
        run_cmd tar -xzf "$OPENSSL_TAR"
        cd "openssl-${OPENSSL_VER}"
        echo "Configuring OpenSSL (static)"
        run_cmd ./config no-shared no-tests
    ...
    # Copy Lua files
    run_cmd cp lib/ssl/src/ssl.lua lib/ssl/init.lua
    run_cmd cp lib/ssl/src/https.lua lib/ssl/

    # Compile SSL Core
    # Link against local OpenSSL static libs
    run_cmd gcc -O2 -shared -fPIC -Isrc/ -Ilib/ssl/src -Ilib/socket/src \
    ...
    if [ $? -eq 0 ]; then
        ...
        cd lib/ssl
        run_cmd ln -sf core.so context.so
        run_cmd ln -sf core.so x509.so
        run_cmd ln -sf core.so config.so
        cd ../..
...
if [ -d "lib/static" ]; then
    ...
    run_cmd cp lib/static/static.lua lib/static/init.lua
    run_cmd chmod +x lib/static/init.lua
