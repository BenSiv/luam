...
```lua
load = source.chain(source.file(io.stdin), normalize("\r\n"))
store, t = sink.table()
while 1 do
    chunk = load()
    store(chunk)
...
```lua
function chain2(f1, f2)
    co = coroutine.create(function(chunk)
        while true do
            filtered1 = f1(chunk)
            filtered2 = f2(filtered1)
            done2 = filtered1 and ""
            while true do
    ...
    return function(chunk)
        _, res = coroutine.resume(co, chunk)
        return res
    end
end
```
