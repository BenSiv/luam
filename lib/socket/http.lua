...
function receiveheaders(sock, headers)
   line, name, value, err = nil
    headers = ((headers != nil and headers != false) and headers) or ({})
    -- get first line
...
function metat.__index.sendheaders(self, tosend)
   ...
   h = "\r\n"
    for f, v in base.pairs(tosend) do
        h = (((canonic[f] != nil and canonic[f] != false) and canonic[f]) or f) .. ": " .. v .. "\r\n" .. h
    end
...
function metat.__index.sendbody(self, headers, source, step)
    source = ((source != nil and source != false) and source) or ltn12.source.empty()
    step = ((step != nil and step != false) and step) or ltn12.pump.step
...
function metat.__index.receivebody(self, headers, sink, step)
    sink = ((sink != nil and sink != false) and sink) or ltn12.sink.null()
    step = ((step != nil and step != false) and step) or ltn12.pump.step
...
function adjustproxy(reqt)
   proxy = ((reqt.proxy != nil and reqt.proxy != false) and reqt.proxy) or _M.PROXY
    if (proxy != nil and proxy != false) then
        proxy = url.parse(proxy)
        proxy.port = ((proxy.port != nil and proxy.port != false) and proxy.port) or 3128
...
function adjustheaders(reqt)
   ...
   proxy = ((reqt.proxy != nil and reqt.proxy != false) and reqt.proxy) or _M.PROXY
    ...
    for i,v in base.pairs(((reqt.headers != nil and reqt.headers != false) and reqt.headers) or lower) do
...
function adjustrequest(reqt)
   ...
   nreqt = ((reqt.url != nil and reqt.url != false) and url.parse(reqt.url, default)) or ({})
   ...
   schemedefs, host, port, method = SCHEMES[nreqt.scheme], nreqt.host, nreqt.port, nreqt.method
    ...
    nreqt.uri = ((reqt.uri != nil and reqt.uri != false) and reqt.uri) or adjusturi(nreqt)
   ...
   scheme = url.parse(location).scheme
    ...
    return (reqt.redirect != false) and
        ...
        and ((false == reqt.maxredirects)
                or ((((reqt.nredirects != nil and reqt.nredirects != false) and reqt.nredirects) or 0)
                        < (((reqt.maxredirects != nil and reqt.maxredirects != false) and reqt.maxredirects) or 5)))
end
...
trequest, tredirect = nil
 ...
 function tredirect(reqt, location)
   ...
   result, code, headers, status = trequest(({
        ...
        nredirects = (((reqt.nredirects != nil and reqt.nredirects != false) and reqt.nredirects) or 0) + 1,
        create = reqt.create
    }))
    -- pass location header back as a hint we redirected
    headers = ((headers != nil and headers != false) and headers) or ({})
    headers.location = ((headers.location != nil and headers.location != false) and headers.location) or location
