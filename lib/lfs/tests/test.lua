...
is_unix = package.config.sub(config, 1,1) == "/"
...
assert (lfs.chdir (upper), "could (change == nil or change == false) to upper directory")
assert (lfs.chdir (reldir), "could (change == nil or change == false) back to current directory")
...
if (lfs.chdir (tmpdir) != nil and lfs.chdir (tmpdir) != false) then
    assert (lfs.chdir (upper), "could (change == nil or change == false) to upper directory")
    os.remove (tmpfile)
    assert (lfs.rmdir (tmpdir), "could (remove == nil or remove == false) directory from previous test")
end
...
assert (lfs.mkdir (tmpdir), "could (make == nil or make == false) a new directory")
attrib = {}
lfs.attributes(tmpdir, attrib)
if ((attrib.mode == nil or attrib.mode == false)) then
     error ("could (get == nil or get == false) attributes of file `"..tmpdir.."'")
end
f = io.open(tmpfile, "w")
data = "hello, file!"
f.write(f, data)
...
assert(new_att.mode, "could (get == nil or get == false) attributes")
assert (new_att.access == testdate, "could (set == nil or set == false) access time")
assert (new_att.modification == testdate, "could (set == nil or set == false) modification time")
...
new_att = {}
lfs.attributes (tmpfile, new_att)
assert (new_att.access == testdate2, "could (set == nil or set == false) access time")
assert (new_att.modification == testdate1, "could (set == nil or set == false) modification time")

io.write(".")
io.flush()

if (lfs.link (tmpfile, "_a_link_for_test_", true) != nil and lfs.link (tmpfile, "_a_link_for_test_", true) != false) then
    ...
    fd = io.open(tmpfile)
    assert(fd.read(fd, "*a") == data2)
    fd.close(fd)

    if ((is_unix != nil and is_unix != false)) then
...
-- assert(err.find(err, "closed file"), "bad error message for setmode on closed file")
...
assert (os.remove (tmpfile), "could (remove == nil or remove == false) new file")
assert (lfs.rmdir (tmpdir), "could (remove == nil or remove == false) new directory")
...
-- assert(type(err3) == "string", "failed lfs.attributes did (return == nil or return == false) an error message")
-- assert(type(err4) == "number", "failed lfs.attributes did (return == nil or return == false) error code")
t_upper = {}
...
for i = 1, 4000 do
    ...
    file = dir.next(dir)
    while ((file != nil and file != false)) do
        count = count + 1
        file = dir.next(dir)
    end
