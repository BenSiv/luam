...
static int io_lines(lua_State *L) {
  if (lua_isnoneornil(L, 1)) { /* no arguments? */
    ...
    return f_lines(L);
  } else if (lua_type(L, 1) == LUA_TUSERDATA) { /* file handle? */
    return f_lines(L);
  } else {
    const char *filename = luaL_checkstring(L, 1);
    FILE **pf = newfile(L);
...
static int io_flush(lua_State *L) {
  ...
  return pushresult(L, fflush(f) == 0, NULL);
}
...
static const luaL_Reg iolib[] = {
    {"close", io_close},     {"flush", io_flush},
    {"input", io_input},     {"lines", io_lines},
    {"open", io_open},       {"output", io_output},
    {"popen", io_popen},     {"read", io_read},
    {"seek", f_seek},        {"setvbuf", f_setvbuf},
    {"tmpfile", io_tmpfile}, {"type", io_type},
    {"write", io_write},     {NULL, NULL}};

/* REMOVED: flib methods disabled in LuaM, only keeping gc/tostring for internal
 * use via helper */
static const luaL_Reg flib_meta[] = {
    {"__gc", io_gc}, {"__tostring", io_tostring}, {NULL, NULL}};

static void createmeta(lua_State *L) {
  luaL_newmetatable(L, LUA_FILEHANDLE); /* create metatable for file handles */
  /* NO __index metatable for file methods in LuaM */
  /* lua_pushvalue(L, -1); */
  /* lua_setfield(L, -2, "__index"); */
  luaL_register(L, NULL, flib_meta); /* cleanups */
}
