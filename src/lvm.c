...
static void traceexec(lua_State *L, const Instruction *pc) {
  ...
  if (mask & LUA_MASKLINE) {
    ...
    int npc = pcRel(pc, p);
    int newline = get_line_info(p, npc);
    /* call linehook when enter a new function, when jump back (loop),
       or when enter a new line */
    if (npc == 0 || pc <= oldpc || newline != get_line_info(p, pcRel(oldpc, p)))
...
int luaV_equalval(lua_State *L, const TValue *t1, const TValue *t2) {
  ...
  case LUA_TUSERDATA: {
    if (uvalue(t1) == uvalue(t2))
      return 1;
    /* metatable field removed - metatables disabled, direct comparison only */
    return 0;
  }
  case LUA_TTABLE: {
    ...
    case OP_LEN: {
      ...
      case LUA_TTABLE: {
        Protect(if (!call_binTM(L, rb, luaO_nilobject, ra, TM_LEN))
                    setnvalue(ra, cast_num(luaH_getn(hvalue(rb))));) break;
      }
      case LUA_TSTRING: {
